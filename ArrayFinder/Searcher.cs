...
public class Searcher
{
    public static void Find(string path)
    {
        char wall = ' ';
        int start = 3 - 1;
        string[][] LineArray = Searcher.ArrayOfArray(path, wall);
        for (int i = 0; i < LineArray[0].Length; i++)
        {
            Console.Write("{1}.{0} ", LineArray[0][i], i + 1);
        }
        Console.Write("{0}.Вывести всю таблицу", LineArray[0].Length + 1);
        int numofcol;
        do
        {
            numofcol = Input.Int("\nВведите номер колонки: ") - 1;
            if ((numofcol >= LineArray[0].Length + 1) || (numofcol < 0))
            {
        ...
        while ((numofcol >= LineArray[0].Length + 1) || (numofcol < 0));
        if (numofcol == LineArray[0].Length)
        {
            Console.ForegroundColor = ConsoleColor.Cyan;
            Output.ArrayOfArray(LineArray);
            Console.ForegroundColor = ConsoleColor.Gray;
        }
        else
        {
    ...
    public static string[][] ArrayOfArray(string path, char wall)
    {
        ...
        {
            LineArray[i] = ArrayOfFile[i].Split(wall);
        }
        return LineArray;
    }
